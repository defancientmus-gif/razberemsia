...
3. Обычное сохранение заметки должно отправлять идею в `texts/IDEAS_NEAR_TERM.md`, если контекст явно идеен.
4. `texts/` остается единственной активной папкой памяти.
5. Философию и UI-стиль не терять: перед дизайном читать `texts/UI_PHILOSOPHY.md`.

## Следующие кодовые задачи

1. После деплоя `rz-v277` провести живой тест `идея` -> `save_idea` -> GitHub.
...
3. Проверить живую PWA после деплоя `rz-v277`.
