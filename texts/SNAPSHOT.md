...
Обновлено: 2026-05-30 после локального фикса `rz-v281`.

Этот файл фиксирует факты, а не желания. Если есть сомнение, проверять код и `git status`.

## Текущий деплой

- **SW cache:** `rz-v281` в `sw.js`.
- **Последний runtime-коммит:** проверить `git log -1 --oneline`.
- **Текущая правка:** Router v1 для голосового агента: профили контекста `none/light/notes/deep`, меньше лишней памяти и уточнений.
