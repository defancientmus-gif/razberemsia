...
Обновлено: 2026-05-31 после локального фикса `rz-v282`.

Этот файл фиксирует факты, а не желания. Если есть сомнение, проверять код и `git status`.

## Текущий деплой

- **SW cache:** `rz-v282` в `sw.js`.
- **Последний runtime-коммит:** проверить `git log -1 --oneline`.
- **Текущая правка:** вернулся видимый бейдж версии рядом с названием; карточка агента открывает созданную заметку/напоминание по `id`, а не по первому элементу ленты.
- **Edge Function `ai`:** задеплоена 2026-05-30 после правки Router v1.
