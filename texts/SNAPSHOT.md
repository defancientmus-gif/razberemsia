...
- **Последний деплой:** 2026-07-08 · rz-v407 · fix: заметки не синхронились между устройствами (push перестал слепо перезаписывать облако)
...
- Попутно: `deploy.sh` искал в SNAPSHOT.md строку `- **Последний деплой:**` для авто-обновления,
