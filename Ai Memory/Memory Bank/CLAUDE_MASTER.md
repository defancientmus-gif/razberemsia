...
## Архитектура папок: теги не создают папки

### Принцип

```
Папки (label) — ФИКСИРОВАННЫЕ, 8 штук навсегда
└── Теги (aiTags[]) — ГИБКИЕ, хранятся внутри заметки

AI возвращает теги → TAG_TO_CAT считает веса → 1 победитель → label заметки
```

Папки никогда не создаются автоматически. AI только определяет, в какую из 8 положить заметку.

### TAG_TO_CAT — таблица весов (реализована в index.html)

| Тег | Папка |
|-----|-------|
| здоровье, врач, давление, таблетки, аптека, больница... | 🏥 здоровье |
| покупки, магазин, список, продукты, доставка... | 🛒 покупки |
| семья, мама, папа, позвонить, написать, внуки... | 👤 контакт |
| встреча, событие, поездка, праздник, расписание... | 📅 событие |
| идея, мысль, план, желание, проект, концепция... | 💡 идея |
| рецепт, готовить, блюдо, ингредиенты... | 🍽️ рецепт |
| адрес, улица, маршрут, навигатор... | 📍 адрес |
| всё остальное | 📝 заметка |

### Алгоритм выбора папки

```
AI вернул теги: ["давление", "врач", "срочно"]

Подсчёт весов:
  давление → здоровье  +1
  врач     → здоровье  +1
  срочно   → не в таблице → 0

Итог: здоровье=2
Победитель → 🏥 Здоровье ← label заметки
```

### Что хранится в заметке

```json
{
  "id": "uuid",
  "title": "Записаться к кардиологу...",
  "body": "полный текст",
  "label": "здоровье",
  "aiTags": ["давление", "врач", "кардиолог", "срочно"],
  "aiSummary": "Нужно записаться к кардиологу из-за давления",
  "reminder": "2026-05-20T09:00",
  "createdAt": 1234567890
}
```

`label` — папка (одна, фиксированная)
`aiTags` — все теги для поиска и отображения пилюлями
`aiSummary` — краткое изложение для ai_memory

### UX при авто-категории

Когда AI применяет категорию автоматически:
- Категория меняется тихо в кнопке категории
- Под кнопкой появляется подсказка на 5 секунд: `✦ AI предложил: 🏥 здоровье`
- Пользователь может поменять вручную в любой момент
- Если пользователь уже выбрал категорию вручную — AI не перезаписывает

### Оптимизация памяти ai_memory через теги

При сохранении в `ai_memory` использовать структуру:

```json
{
  "cluster": "здоровье",
  "summary": "Нужно записаться к кардиологу из-за давления",
  "tags": ["давление", "врач"],
  "importance": 4,
  "noteId": "uuid"
}
...
Это позволяет при следующем запросе подгружать только память по нужному кластеру — не гонять всё подряд.
